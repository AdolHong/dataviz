...
class ParameterDomainContract:
    ...
    def as_dict(self) -> dict[str, Any]:
        return {
            "schema": "dataviz/parameter-domain-contract/v1",
            "dependencies": {
            ...
            "domain_inputs": {
                key: list(value) for key, value in self.domain_inputs.items()
            },
        }
...
def compile_parameter_domain_contract(
    ...
    for domain_id, (_, domain) in dashboard.parameter_domains.items():
        inputs = {
            query_input_parameter(binding) for binding in domain.query_inputs.values()
        }
    ...
    def put(self, key: str, frame: pd.DataFrame, content_hash: str) -> None:
        with self._lock:
            self._entries[key] = _CacheEntry(
                frame.copy(deep=True), time.monotonic(), content_hash
            )
...
class ParameterDomainResolution:
    ...
    def as_dict(self) -> dict[str, Any]:
        generation_payload = {
            key: value.get("content_hash", "") for key, value in self.domains.items()
        }
        generation = hashlib.sha256(
            json.dumps(generation_payload, sort_keys=True).encode("utf-8")
        ).hexdigest()[:16]
        return {
            "schema": "dataviz/parameter-domain-resolution/v1",
            "status": "ready",
...
def _execute_domain(
    ...
    *,
    ...
    query_inputs = project_query_inputs(
        definition.query_inputs, parameters, parameter_intents
    )
    ...
    cached = None if refresh or cache is None or definition.cache.mode == "none" else cache.get(
    ...
    for parent, binding in options.depends_on.items():
        ...
        filtered = filtered[
            filtered[binding.field].map(
                lambda item, allowed=signatures: False
                if _is_null(item)
                else json_value_signature(_json_scalar(item)) in allowed
            )
        ]
    if options.sort_field:
        filtered = filtered.sort_values(options.sort_field, kind="stable")
    ...
    *,
    ...
    def initial() -> tuple[Any, str]:
        ...
        return normalize_control_value(
            definition, value, enforce_required=True
        ), resolved_intent
    ...
    try:
        normalized = normalize_control_value(
            definition, raw_value, enforce_required=False
        )
    ...
    if preserve_unavailable:
        ...
        return normalize_control_value(
            definition, normalized, enforce_required=True
        ), (
            "all_available"
    ...
    if strict and len(retained) != len(items):
        invalid = [
            item for item in items if json_value_signature(item) not in signatures
        ]
        raise ExecutionFailure(
    ...
    if definition.type == "multiple_select":
        if items and not retained:
            return initial()
        return normalize_control_value(
            definition, retained, enforce_required=True
        ), "explicit"
    if items and retained:
        return normalize_control_value(
            definition, retained[0], enforce_required=True
        ), "explicit"
    return initial()
    ...
    for parameter_id in contract.order:
        ...
        if not isinstance(options, ParameterDomainOptionDefinition):
            value = resolve_query_parameter_values(
                [definition],
                {parameter_id: provided[parameter_id]}
        ...
        input_signature = json.dumps(
            domain_inputs, sort_keys=True, ensure_ascii=False, default=str
        )
        ...
        parameter_choices = _project_choices(
            definition, frames[frame_key], resolved
        )
        ...
        if preserve_unavailable:
            parameter_choices = _with_unavailable_choices(
                definition, parameter_choices, value
            )
            choices[parameter_id] = parameter_choices
        resolved[parameter_id] = value
