...
function galleryViewMarkup(state, index) {
  ...
  return `<article class="dv-view gallery-state-view" data-view-id="gallery-state-view-${index}" data-view-status="${state}">
    ...
    <div class="dv-view-body">${isMetric
      ? `<div class="dv-metric"><div class="dv-metric__primary"><strong class="dv-metric__value">${galleryEscape(galleryStateCopy(state, 'view'))}</strong></div><span class="dv-metric__caption">selected revenue</span></div>`
...
.gallery-state-card{display:flex;flex-direction:column;min-height:330px;background:color-mix(in srgb,var(--dv-panel) 96%,transparent);border:1px solid var(--dv-line);border-radius:var(--dv-radius);box-shadow:var(--dv-shadow);overflow:hidden;scroll-snap-align:start}
