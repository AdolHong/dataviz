...
0.26.1：侧栏搜索、默认折叠、根目录/文件夹递归展开与折叠；搜索期间禁止拖动。复用本轮相关非浏览器及 Chromium 验证，发布仅执行版本/构建检查和安装冒烟，不重跑全量。
...
当前工作树版本：`0.26.1`。下方旧版本记录仅为历史，不代表本次验证结果。本文件区分本地发行构建、工作树变更与待验证事项。架构理由见 [ARCHITECTURE](ARCHITECTURE.md)，视觉规范见 [DESIGN](DESIGN.md)，代码见 [实现索引](docs/product-architecture.md)，发行历史见 [CHANGELOG](CHANGELOG.md)。
