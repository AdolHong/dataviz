...
Dashboard              dataviz/dashboard/v20
...
Browser Runtime        dataviz/runtime/v15
...
`presentation.shell` 拥有共享组件布局和右侧 Controls 样式；Server Shell 拥有 Parameters/Controls 操作面板、导航及焦点。Parameters 与 Controls 共用右侧可切换面板，Header 保留独立入口与 Run，字段不再位于 Workbench 顶部的 Query Card。面板宽度、层级视觉与响应式规则以 DESIGN.md 为准，避免在架构文档重复维护。Dashboard/Section/View Controls 使用同一个 canonical 状态；Section/View 默认侧栏，显式 Popover 为展示覆盖，不创建第二套状态或触发额外 Query。独立 HTML 的查询参数是只读证据，不支持重新查库或工作台导航；可用 Controls 的状态仍由 Runtime 管理。Server 同源 Canvas 的快捷键由 Shell 捕获，未处理事件保留给组件/Runtime；等待页与完成页遵守相同焦点、输入保护和消息身份边界。Schema 和 CLI 文档负责作者契约，视觉文档不应成为另一份执行协议。
