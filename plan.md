...
当前基线：Package `0.2.0`、Python 3.11–3.14、Dashboard `dataviz/dashboard/v2`、Browser Runtime `dataviz/runtime/v2`、Component Registry `3.0.0`。项目尚未投入生产，因此只接受当前严格契约，不保留旧字段 alias、自动迁移或第二套 Runtime。

## 已完成的核心能力

### 数据、计算与状态

- [x] Query Parameter 提交后创建不可变 Query Run；Selection 只筛选已有样本；Compute Parameter 只失效声明依赖它的 Interactive Transform。
...
- [x] Server 与 HTML 共用 Renderer、Selection、Compute、Overlay 和内容绑定实现。
...
- [x] Gallery 提供 Selector、Compute、View、Section 的七状态矩阵，以及实际包含 10、100、1,000 个原生选项的 Select Story；1,000 选项搜索覆盖全量且增强 DOM 有界。
...
- [x] Selection、Compute 与 Output delta 明确区分“全部/部分/无变化”；无关 Output 不再重启 active Transform，未变化 Output 不再重复传播或重绘。
...
- [x] Selector、Compute、View、Section 七状态矩阵和 Select 10/100/1,000 真实选项 Story 已进入 Gallery 与 Chromium 契约测试。
