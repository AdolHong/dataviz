...
class Executor:
    ...
    def run(
        ...
        *,
        ...
        if selections is not None and filters is not None:
            raise ExecutionFailure("Use selections or legacy filters, not both")
        selection_values, _ = resolve_selection_values(
            dashboard.definition, selections if selections is not None else filters
        )
        plan = compile_plan(dashboard, source_targets=source_targets)
