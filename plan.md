...
0.25.5：统一 W Controls / E Parameters 快捷键，修正服务端冷启动与 Renderer 就绪测试前提；完整本地测试与定向复验见 `docs/release-0.25.5.md`。以下历史段落不代表当前全量结果。
...
当前工作树版本：`0.25.5`，本次完整运行及定向修正结果见 [0.25.5 验收记录](docs/release-0.25.5.md)。本文件区分本地发行构建、工作树变更与待验证事项，不以完成过的历史阶段作为未来计划。架构理由见 [ARCHITECTURE](ARCHITECTURE.md)，视觉规范见 [DESIGN](DESIGN.md)，代码见 [实现索引](docs/product-architecture.md)，发行历史见 [CHANGELOG](CHANGELOG.md)。
