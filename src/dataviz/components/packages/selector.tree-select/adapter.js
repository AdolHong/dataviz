(function registerTreeSelect(global) {
  ...
  if (!api) return;
  api.register('tree-select', ({selector, input, mount}) => {
    let levels = [];
    ...
    const paths = () => {
      const values = root.selectorPathOptions?.({selector, input, levels}) || [];
    ...
    const rebuildNative = values => {
      const key = selector.closest('[data-selection-key]')?.dataset.selectionKey;
      const previous = input.options.length
        ? new Set(api.selectedOptions(input).map(option => option.value))
        : new Set((global.dataviz?.selections?.[key] || []).map(JSON.stringify));
      input.replaceChildren(...values.map(path => {
    ...
    function render(rebuild = true) {
      ...
      if (!initializedExpansion) {
        const depth = Math.min(
          Math.max(0, Number(selector.dataset.defaultExpandDepth || 0)),
      ...
      if (query) {
        ...
        const walk = (prefix, depth) => {
          ...
          children.forEach(path => {
            ...
            if (!leaf && selector.dataset.hierarchySelection === 'cascade' && input.multiple) {
      ...
      api.renderSummary(
        ...
        api.hierarchySummaryLabels(values, selected, selector.dataset.checkedStrategy || 'child', separator),
      );
