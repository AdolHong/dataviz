...
from dataviz.workspace.models import DashboardDefinition, PresentationDefinition, SelectionDefinition
...
LEGACY_WORKSPACE = Path(__file__).parents[1] / "examples" / "sales-workspace"
...
def test_repeat_templates_share_one_dataset_and_render_dynamic_instances(tmp_path: Path):
    ...
    artifact = result.nodes["source:store-sales"].artifacts[0]
    ...
    assert report.count('"store_id": "S001"') == 12
...
def test_cli_docs_provide_onboarding_chart_recipes_and_error_recovery():
    ...
    assert chart_docs["field_matrix"]["heatmap"]["required"] == ["source", "x", "y", "z"]
...
def test_optional_presentation_overrides_ids_without_changing_logic():
    ...
    assert all(section.template == "grid" for section in dashboard.logic_definition.sections)
    ...
    assert dashboard.definition.layout.items[0].widget == "revenue-trend"
    assert dashboard.definition.layout.items[0].width == 8
    assert dashboard.definition.layout.items[0].min_height == 420
    assert dashboard.definition.views[0].source == dashboard.logic_definition.views[0].source
    assert validate_workspace(workspace) == []
...
def test_selector_presentation_templates_are_visual_only():
    presentation = PresentationDefinition.model_validate(
        {
            ...
            "selectors": {
                "dashboard:sales-overview/region": {"template": "dropdown"},
    ...
    assert presentation.selectors["dashboard:sales-overview/region"].template == "dropdown"
    product = presentation.selectors["view:sales-detail/product"]
    assert product.template == "searchable"
...
def test_portable_selector_markup_supports_search_and_hidden_unavailable_options():
    ...
    assert 'data-selector-template="searchable"' in markup
...
def test_stale_presentation_references_warn_and_do_not_break_dashboard(tmp_path: Path):
    ...
    presentation["views"]["deleted-view"] = {"width": 12}
    presentation["sections"]["deleted-section"] = {"template": "single"}
    presentation["selectors"] = {"view:deleted-view/region": {"template": "dropdown"}}
...
def test_presentation_can_place_file_based_legacy_views(tmp_path: Path):
    root = tmp_path / "workspace"
    shutil.copytree(LEGACY_WORKSPACE, root)
...
""",
    ...
    revenue_layout = next(item for item in dashboard.definition.layout.items if item.widget == "revenue")

    assert revenue_layout.width == 9
    assert "dv-view--chart" in revenue_layout.css_class
    assert not [
        item
...
def test_declarative_selection_contract_uses_field_binding():
    ...
    assert set(contract) == set(dashboard.widgets)
...
def test_declarative_renderer_limits_selection_redraw_to_affected_views():
    ...
    assert "affected.has(view.id)" in rendered
    assert "const datavizAffectedViewIds = changedKeys =>" in rendered
    assert "window.datavizClient.render(window.dataviz, window.dataviz.renderContext)" in rendered


def test_legacy_filter_schema_loads_but_serializes_as_selection():
    definition = DashboardDefinition.model_validate(
        {
            "id": "compat",
            "title": "Compatibility",
            "dashboard_filters": [{"id": "region", "type": "multi_select"}],
            "sections": [
                {
                    "id": "main",
                    "title": "Main",
                    "filters": [{"id": "period", "type": "date_range"}],
                    "views": [
                        {
                            "widget": "detail",
                            "filters": [{"id": "status"}],
                            "filter_bindings": {"status": "order_status"},
                        }
                    ],
                }
            ],
    ...
    assert definition.dashboard_selections[0].mode == "include"
    assert definition.dashboard_selections[0].cascade is True
    assert definition.sections[0].selections[0].id == "period"
    assert definition.sections[0].views[0].selection_bindings["status"] == "order_status"
    assert definition.canvas.client_selections is True
    assert "dashboard_selections" in dumped and "dashboard_filters" not in dumped
    assert "selections" in dumped["sections"][0] and "filters" not in dumped["sections"][0]


def test_selection_runtime_supports_explicit_exclude_mode(tmp_path: Path):
    workspace = load_workspace(WORKSPACE)
    dashboard = workspace.dashboard("sales-overview")
    dashboard.definition.dashboard_selections[0].mode = "exclude"
    result = Executor(workspace).run("sales-overview", selections={"region": ["East"]})
    report = CanvasRenderer(workspace).render(dashboard, result, asset_mode="inline")

    assert '"mode": "exclude"' in report
    assert "item.definition?.mode === 'exclude' ? !matched : matched" in report
...
def test_query_parameter_changes_inline_sql_dataset():
    ...
    artifact = result.nodes["source:sales"].artifacts[0]
    frame = ArtifactStore(workspace.root, result.run_id).read_table(artifact)
...
def test_default_renderer_builds_templates_and_portable_report(tmp_path: Path):
    ...
    assert '"sources": {"sales": [' in report
    ...
    assert "if (typeof viewer.flush === 'function') await viewer.flush()" in report
