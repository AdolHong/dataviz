(function exposeSelectSelector(global) {
  'use strict';
  const root = global.datavizComponents;
  const api = root?.selectors;
  if (!api || root.createSelectSelector) return;

  const enabledByMode = (mode, count, threshold) => (
    mode === 'always' || (mode !== 'never' && count >= threshold)
  );

  root.createSelectSelector = function createSelectSelector({selector, input, mount}) {
    const rowHeight = Math.max(28, Number(selector.dataset.itemHeight || 38));
    const viewportHeight = Math.max(rowHeight * 3, Number(selector.dataset.viewportHeight || 304));
    const overscan = Math.max(2, Number(selector.dataset.overscan || 5));
    ...
    search.dataset.selectorSearch = '';
    search.placeholder = selector.dataset.searchPlaceholder || 'Search options…';
    ...
    empty.textContent = selector.dataset.emptyText || 'No matching options';
    empty.hidden = true;
    ...
    all.type = 'button';
    all.textContent = selector.dataset.selectAllLabel || 'Select all';
    ...
    const overlay = api.floating(picker, trigger, panel, {
      width: Number(selector.dataset.overlayWidth || 440),
      focus: search,
      onOpen: () => sync(),
    });
    ...
    function render() {
      ...
      if (virtual) {
        ...
        renderedOptions.forEach((option, index) => {
          ...
          if (group && group !== previousGroup) {
            ...
            heading.dataset.selectorGroup = group;
      ...
      all.textContent = allCandidatesSelected
        ? selector.dataset.invertLabel || 'Invert'
        : selector.dataset.selectAllLabel || 'Select all';
      all.dataset.action = allCandidatesSelected ? 'invert' : 'select-all';
      all.disabled = input.disabled || candidates.length === 0 || (!allCandidatesSelected && required > remaining);
      all.title = !allCandidatesSelected && required > remaining
        ? `Selection limit is ${maxSelected}; narrow the search or clear values first.`
        : '';
      api.renderSummary(trigger, input, selector, selector.dataset.placeholder || 'Choose…');
    ...
    function sync() {
      ...
      const searchEnabled = enabledByMode(
        selector.dataset.searchMode || 'auto',
        allOptions.length,
        Math.max(0, Number(selector.dataset.searchThreshold || 9)),
      );
      virtual = enabledByMode(
        selector.dataset.virtualMode || 'auto',
        allOptions.length,
        Math.max(1, Number(selector.dataset.virtualThreshold || 200)),
      );
      search.hidden = !searchEnabled;
      if (!searchEnabled) search.value = '';
      const query = search.value.trim().toLocaleLowerCase();
      filtered = allOptions.filter(option => {
        if (option.disabled && selector.dataset.showUnavailable !== 'true') return false;
        if (selector.dataset.hideSelected === 'true' && option.selected) return false;
